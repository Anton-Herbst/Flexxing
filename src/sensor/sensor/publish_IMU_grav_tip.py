...
"""
* This script simply grabs processed sensor data (IMU Gravity) and transforms it the local coordinate system.
...
class Publisher_IMU_grav_tip(Node):

    # * function called on creation
    def __init__(self):
        
        # "it insists upon itself"
        super().__init__('publish_IMU_grav_tip')
        ...
        self.subscription_grav       = self.create_subscription(Vector3, f'/teensy_hub/IMU_gravity_data_{self.tip}', self.callback_IMU_grav, qos)
        #create publisher to release transformed Vector
        self.publisher              = self.create_publisher(Vector3, '/pc/transformed_IMU_grav_tip', 10)

        # * Parameter from calibration (local YAML)
        # read in the config file with the calibrated values
        config_file_path_IMU = os.path.expanduser('~/.ros/sensor/calibration/IMU_cal_tip.yaml')
        ...
        self.matrix = np.array(extracted_data_IMU['IMU_calibration_tip']['grav'])

    # * callback function of sensors
    # whenever new data arrives do some math and publish the results straightaway
    def callback_IMU_grav(self, msg: Vector3) -> None: self.transform(msg)
    ...
    def transform(self, raw: Vector3) -> None:
        ...
        transformed = self.matrix @ measured
        # now normalize it
