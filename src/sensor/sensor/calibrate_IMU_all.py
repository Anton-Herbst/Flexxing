...
class CalibrateIMUSensor(Node):
    ...
    def save_yaxis(self) -> None:
        ...
        for key in range(1,5):
            ...
            v1_acc, v1_grav = self.get_axis_vector(key, 'z-axis')
            # get the saved x-axis
            v2_acc, v2_grav = self.get_axis_vector(key, 'x-axis')
            ...
            self.set_axis_vector(v1x2_acc, 'acc', key, 'y-axis')
            self.set_axis_vector(v1x2_grav, 'grav', key, 'y-axis')

    # * function to save new xaxis
    def save_xaxis(self) -> None:
        for key in range(1, 5):
            # get the mean value
            v1_acc, v1_grav = self.evaluate_samples(key)
            # get the last saved measurement (which was the negative z-axis)
            v2_acc, v2_grav = self.get_axis_vector(key, 'z-axis')
            ...
            self.set_axis_vector(v2x1_acc, 'acc', key, 'x-axis')
            self.set_axis_vector(v2x1_grav, 'grav', key, 'x-axis')
            # clean up crew
            self.samples['acc'][key].clear()
            self.samples['grav'][key].clear()
    ...
    def get_rot_matrix(self, topic: str, key:int) ->  list:
        return [[self.axis[topic][key]['x-axis']['x'], self.axis[topic][key]['y-axis']['x'], self.axis[topic][key]['z-axis']['x']],
                [self.axis[topic][key]['x-axis']['y'], self.axis[topic][key]['y-axis']['y'], self.axis[topic][key]['z-axis']['y']],
                [self.axis[topic][key]['x-axis']['z'], self.axis[topic][key]['y-axis']['z'], self.axis[topic][key]['z-axis']['z']]]
        ...
        with open(file = local_config_file, mode = 'w') as file_handle:
            ...
            yaml.safe_dump(
                ...
                data = { # dont forget to add the namespace
                    'IMU_calibration_all': {
                        topic: {
                            key: self.get_rot_matrix(topic, key) for key in range(1, 5)
                        }for topic in ('acc', 'grav')
                    }},)
