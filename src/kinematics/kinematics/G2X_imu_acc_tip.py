...
class G2X_imu_acc_tip(Node):
    ...
    def __init__(self):
        ...
        super().__init__('forward_PCC_G2X_imu_acc_tip')

        # * ROS related
        # subscribe to incoming generalized coordinates for the tip from sensor/gen_.._imu_acc_tip
        self.subscription = self.create_subscription( Float64MultiArray, '/pc/gen_coords_imu_acc_tip', self.callback_gen_coords_tip, 10 )
    ...
    def callback_gen_coords_tip(self, msg: Float64MultiArray) -> None:
        ...
        # calulate stuff from it
        # self.get_rotation_matrix(delta_x, delta_y, delta) # TODO if you have time show the orientation of the tip as well, not only the position
        transformation_vector = self.get_transformation_vector(delta_x, delta_y, delta)
        # prepare a message
        msg_pub = Vector3()
        # calculate the endeffector position and publish it
        msg_pub = transformation_vector
        # fire it away
