...
class Inverse_PCC_G2L_mag_all(Node):

    # * function called on creation
    def __init__(self):
        # it insists upon itself
        super().__init__('inverse_PCC_G2L_mag_all')

        # * ROS related
        # subscribe to incoming generalized coordinates for each segment published by sensor/gen_coords_mag
        self.subscription_bot = self.create_subscription(Float64MultiArray, '/pc/gen_coords_mag_bot', lambda msg: self.callback_gen_coords(msg, 'bot'), 10)
        self.subscription_top = self.create_subscription(Float64MultiArray, '/pc/gen_coords_mag_top', lambda msg: self.callback_gen_coords(msg, 'top'), 10)
...
def main():
    rclpy.init()
    mynode = Inverse_PCC_G2L_mag_all()
    rclpy.spin(mynode)
    mynode.destroy_node()
    rclpy.shutdown()
