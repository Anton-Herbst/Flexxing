...
class Inverse_PCC_G2L_controller(Node):
    ...
    def inverse_kinematics_G2L(self, delta_x: float, delta_y: float, segment: str) -> tuple[float, float, float]:
        ...
        if segment == 'top': # -> l4, l5, l6
            ...
            l_4 = self.segment_length - self.d * (1/2 * delta_x+ delta_y * np.sqrt(3)/2)
            l_5 = self.segment_length + self.d * (-1/2 * delta_x+ delta_y * np.sqrt(3)/2)
            l_6 = self.segment_length + self.d * delta_x
            return l_4, l_5, l_6
