...
class Inverse_PCC_G2L_mag_tip(Node):

    # * function called on creation
    def __init__(self):
        # it insists upon itself
        super().__init__('inverse_PCC_G2L_mag_tip')

        # * ROS related
        # subscribe to incoming generalized coordinates for the tip segment from sensor/gen_coords_mag_tip
        ...
        self.publisher = self.create_publisher(Float64MultiArray, '/pc/tendon_length_mag_tip', 10)
...
def main():
    rclpy.init()
    mynode = Inverse_PCC_G2L_mag_tip()
    rclpy.spin(mynode)
    mynode.destroy_node()
    rclpy.shutdown()
